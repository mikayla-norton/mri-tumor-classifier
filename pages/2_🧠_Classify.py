...
file = st.file_uploader("Please upload an MRI scan: ", type=["jpg", "jpeg"])
...
if file is not None:
    ...
    with st.spinner(text="Generating Results... please wait..."):
        ...
        p = max(list(p[0]))
        if p < 0.95:
            st.error("An error has occurred. Are you sure you uploaded an MRI scan? (reminder: the model can only handle a single scan at a time)", icon="🚨")
        else:
